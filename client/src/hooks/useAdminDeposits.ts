...
import { paymentService, type DepositProof, type UpdateProofStatusRequest } from '@/services/payment.service';
...
export const useAdminDeposits = () => {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: ({ proofId, data }: { proofId: string; data: UpdateProofStatusRequest }) =>
  ...
  const approveDeposit = (proofId: string, reason?: string) => {
