...
import { paymentService, type PaymentDetails } from '@/services/payment.service';
...
export const usePayment = () => {
  ...
  const createDeposit = async (amount: number, method: PaymentMethod, referenceNumber?: string) => {
    try {
      if (!method) {
  ...
  const uploadPaymentProof = async (file: File, referenceNumber?: string) => {
